...
MaskingMode = Literal[
    "boolean",# eg: boolean tensor, shape: [1, H, W], dtype: bool
    "scalar",# eg: scalar(0.0 - 1.0) tensor, shape: [1, H, W], dtype: float8/16/32
    "uint8", # eg: uint8 tensor, shape: [1, H, W], dtype: uint8
    "uint16",# eg: uint16 tensor, shape: [1, H, W], dtype: uint16
    "image_luminance", # eg: image mode='L', dtype: uint8
    "image_alpha", # eg: image mode='RGBA', dtype: uint8, alpha channel is the mask
    "image_compound",# eg: each channel is a different mask
]
...
class MaskField(BaseModel):
    ...
    mode: MaskingMode = Field(description="The masking mode specifies how the mask is represented.")
